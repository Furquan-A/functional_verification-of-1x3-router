...
VSIMBATCH1= -c -do  " log -r /* ;coverage save -onexit router_fcov;run -all; exit"
...
help:
	...
	@echo "! run_test	=>  clean, compile & run the simulation for small_pkt_test in batch mode.		!" 
	@echo "! run_test1	=>  clean, compile & run the simulation for medium_pkt_test in batch mode.			!" 
	@echo "! run_test2	=>  clean, compile & run the simulation for large_pkt_test in batch mode.			!"
	@echo "! view_wave1 =>  To view the waveform of small_pkt_test	    						!" 
	@echo "! view_wave2 =>  To view the waveform of medium_pkt_test	    						!" 
	@echo "! view_wave3 =>  To view the waveform of large_pkt_test 	  						!" 
...
run_test_Questa: sv_cmp
	vsim -cvgperinstance $(VSIMOPT) $(VSIMCOV) $(VSIMBATCH1)  -wlf wave_file1.wlf -l test1.log  -sv_seed random  work.top +UVM_TESTNAME=small_pkt_test
	vcover report  -cvg  -details -nocompactcrossbins -codeAll -assert -directive -html router_fcov
	
run_test1_Questa:
	vsim -cvgperinstance $(VSIMOPT) $(VSIMCOV) $(VSIMBATCH2)  -wlf wave_file2.wlf -l test2.log  -sv_seed random  work.top +UVM_TESTNAME=medium_pkt_test
	vcover report  -cvg  -details -nocompactcrossbins -codeAll -assert -directive -html router_fcov
	
run_test2_Questa:
	vsim -cvgperinstance $(VSIMOPT) $(VSIMCOV) $(VSIMBATCH3)  -wlf wave_file3.wlf -l test3.log  -sv_seed random  work.top +UVM_TESTNAME=large_pkt_test
	vcover report  -cvg  -details -nocompactcrossbins -codeAll -assert -directive -html router_fcov
...
run_test_VCS:	clean  sv_cmp_VCS
	./simv -a vcs.log +fsdbfile+wave1.fsdb -cm_dir ./router_fcov +ntb_random_seed_automatic +UVM_TESTNAME=small_pkt_test 
	urg -dir mem_cov1.vdb -format both -report urgReport1
	
run_test1_VCS:	
	./simv -a vcs.log +fsdbfile+wave2.fsdb -cm_dir ./router_fcov +ntb_random_seed_automatic +UVM_TESTNAME=medium_pkt_test 
	urg -dir mem_cov2.vdb -format both -report urgReport2
	
run_test2_VCS:	
	./simv -a vcs.log +fsdbfile+wave3.fsdb -cm_dir ./router_fcov +ntb_random_seed_automatic +UVM_TESTNAME=large_pkt_test 
...
report_VCS:
	urg -dir router_fcov.vdb -dbname merged_dir/merged_test -format both -report urgReport
